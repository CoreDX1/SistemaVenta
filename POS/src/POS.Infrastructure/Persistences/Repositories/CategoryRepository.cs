...
public class CategoryRepository : GenericRepository<Category>, ICategoryRepository
{
    ...
    public async Task<BaseEntityReponse<Category>> ListCategories(BaseFilterRequest filters)
    {
        ...
        response.Items = await Ordering(filters, categories, !(bool)filters.Download!)
            .ToListAsync();
        return response;
    }
    ...
    public async Task<IEnumerable<Category>> ListSelectCategories()
    {
        var categories = await _context.Categories
            .Where(
                x =>
                    x.State.Equals((int)StateTypes.Acitve)
    ...
    public async Task<Category> CategoryById(int categoryById)
    {
        var category = await _context.Categories
            .AsNoTracking()
            .FirstOrDefaultAsync(x => x.CategoryId.Equals(categoryById));
        return category!;
    }
    ...
    public async Task<bool> RemoveCategory(int categoryId)
    {
        var category = await _context.Categories
            .AsNoTracking()
            .SingleOrDefaultAsync(x => x.CategoryId.Equals(categoryId));
